...
def generate_sequence(model, prime_seq, sequence_len, mean_s, stddev_s, architecture, use_stateful=False, add_spectra=False):
    ...
    if use_stateful:
        ...
        for idx in range(sequence_len - prime_seq.shape[1]):
            ...
            predict_seq = model.predict(prime)
            # print("predict_seq.shape ", predict_seq.shape)

            # print("predict_seq ", predict_seq.shape)
            seq_t_plus_1 = predict_seq[0, predict_seq.shape[1] - 1, :]
            # print("seq_t_plus_1 ", seq_t_plus_1.shape)
            t = np.max(np.abs(seq_t_plus_1))
            # print("MAX ", t)
        ...
        # plt.plot(np.reshape(predict_seq, (predict_seq.shape[1] * predict_seq.shape[2])))
        # plt.show()
        if add_spectra:
...
def gen_seq_full(folder_spec, data, model_name, prime_length, num_of_tests, add_spectra=False, mean_std_per_file=False,
    ...
    for test_index in range(num_of_tests):
        ...
        print("mean_x, stddev_x ", mean_x.shape, stddev_x.shape)
        ...
        tmp = np.reshape(sequence_total, sequence_total.shape[1] * sequence_total.shape[2])
        o_filename = gen_directory + "/" + ".".join(f_parts[:-1]) + ".npy"
        with open(o_filename, 'wb') as fs:
            np.savez(fs, tmp)
        gen_filename = gen_directory + "/" + ".".join(f_parts[:-1]) + "_gen.npy"
        with open(gen_filename, 'wb') as fs:
            np.savez(fs, generated_sequence)
