...
class ConvAutoencoder:
    ...
    def __init__(self, train_set, test_set):
        ...
        x = Convolution1D(32, 41, activation='relu', border_mode='same')(input_img)
        x = BatchNormalization()(x)
        x = MaxPooling1D(2, border_mode='valid')(x)
        x = Convolution1D(16, 17, activation='relu', border_mode='same')(x)
        x = BatchNormalization()(x)
        x = MaxPooling1D(2, border_mode='valid')(x)
        ...
        x = BatchNormalization()(x)
        x = UpSampling1D(2)(x)
        x = Convolution1D(32, 41, activation='relu', border_mode='same')(x)
        x = BatchNormalization()(x)
        x = UpSampling1D(2)(x)
    ...
    def test_encoder(self):
        return self.autoencoder.predict(self.test_data)